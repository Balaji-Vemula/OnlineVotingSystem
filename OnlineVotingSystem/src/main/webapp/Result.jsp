...
<%
	...
	System.out.println("out result yes");
	if(session.getAttribute("email")==null)
	{
		System.out.println("in result yes");
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
    ...
    <div class="py-3">
     <h2 class="text-dark text-center font-weight-bold">Result</h2>
    </div>
        <table class="table">
            ...
            <tr>
                ...
                <th class="font-weight-bolder text-center">Voting Counts</th>
                
            </tr>
            </thead>
            <tbody>
            <c:forEach var="results" items="${listResults}">
                <tr>
                    <td class="text-center"><c:out value="${results.c_id}" /></td>
                    <td class="text-center"><c:out value="${results.c_first_name}" /></td>
                    <td class="text-center"><c:out value="${results.c_last_name}" /></td>
                    <td class="text-center"><c:out value="${results.party_name}" /></td>
                    <td class="text-center"><c:out value="${results.c_votes}" /></td>
                 </tr>
              </c:forEach>
              </tbody>
              </table>
